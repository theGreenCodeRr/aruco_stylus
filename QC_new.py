...
def load_camera_calibration():
    cameraMatrix = np.array( #720p webcam
        [
...
def main():
    video_path = 'video/cam2_720p30.mkv'
    local_thickness = 3
    global_thickness = 7
    start_frame = 1
    end_frame = 262
    ...
    while True:
        ...
        # Clear tracking for disappeared markers
        for mid in list(local_last):
        ...
        for mid, pts2d in valid:
            ...
            # Add segment if consecutive
            if mid in local_last:
    ...
    # Save CSVs
    if local_records:
        pd.DataFrame(local_records).to_csv('cam2_qc_local.csv', index=False)
    if global_records:
        pd.DataFrame(global_records).to_csv('cam2_qc_global.csv', index=False)
