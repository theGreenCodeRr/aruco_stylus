...
def visualize(df: pd.DataFrame, out_png: str):
    ...
    plt.plot(df['frame'], df['input_angle'], label='GT')
    plt.plot(df['frame'], df['pred_angle'], label='Pred')
    plt.xlabel('Frame')
...
def main():
    parser = argparse.ArgumentParser(
        description='Angle regression with error stats and visualization'
    )
    parser.add_argument(
        '--data', '-d', default='6DoF_annotated_poses.csv',
        help='Path to the 6DoF CSV file'
    )
    parser.add_argument(
        '--out', '-o', default='predictions.csv',
        help='Output CSV with full results (frame, input_angle, pred_angle, error)'
    )
    parser.add_argument(
        '--pred-out', default='predicted_angles.csv',
        help='CSV file to save predicted and GT angles'
    )
    ...
    # Save predicted and ground-truth angles
    df_pred[['frame', 'input_angle', 'pred_angle']].to_csv(args.pred_out, index=False)
    print(f"Saved predicted and GT angles to {args.pred_out}")
